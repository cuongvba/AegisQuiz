...
```mermaid
graph TB
    subgraph ClientBoundary["LỚP CLIENT TRỰC QUAN (PRESENTATION LAYER)"]
        UI["React 19 + TypeScript + TailwindCSS"]
        WebAudioEngine["Web Audio Synthesizer (0ms Latency SFX)"]
        Canvas60fps["Canvas 60fps Physical Engine (Lucky Wheel)"]
        SignalRClient["SignalR WebSocket Client Channel"]
    end

    subgraph SecurityGateway["CỔNG BẢO MẬT & ĐIỀU PHỐI (API GATEWAY)"]
        AuthMid["JWT + Keycloak OIDC + PKI USB Token Middleware"]
        Controllers[".NET 10 Controllers (RESTful Endpoints)"]
        ArenaHub[".NET 10 SignalR Hub (/hubs/arena)"]
    end

    subgraph CoreEngine["TRÁI TIM ĐIỀU HÀNH (APPLICATION & DOMAIN)"]
        RoomMgr["ArenaRoomManager (In-Memory Microsecond Clock)"]
        PluginMgr["IArenaGamePlugin Engine (6 Gameshow Plugins)"]
        DocxEngine["Docx / WMF Math Parser (OpenXML to LaTeX)"]
        AppBus["CQRS & Domain Event Handlers"]
    end

    subgraph MicroservicesAndData["TẦNG TÍNH TOÁN NẶNG & LƯU TRỮ (PERSISTENCE)"]
        PythonIrt["Python FastAPI Service (IRT 3-PL CAT Engine - Port 8001)"]
        GeminiAi["Google Gemini 1.5 Flash (Socratic AI Mentor)"]
        Postgres[(PostgreSQL 16 Multi-Tenant RLS Database)]
        RedisCache[(Redis Distributed State Cache)]
    end

    ClientBoundary <-->|HTTPS REST & WebSockets| SecurityGateway
    SecurityGateway --> CoreEngine
    CoreEngine <--> PythonIrt
    CoreEngine <--> GeminiAi
    CoreEngine --> Postgres
